...
uint8_t u8g2_esp32_i2c_byte_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) {
	...
	switch(msg) {
		...
		case U8X8_MSG_BYTE_INIT: {
			...
			if ( u8g2_esp32_hal.reset >= 0 ) {
				gpio_set_direction( u8g2_esp32_hal.reset, GPIO_MODE_OUTPUT );
				gpio_set_level( u8g2_esp32_hal.reset, 1 );
			}
		    ...
		    ESP_ERROR_CHECK(i2c_param_config(I2C_MASTER_NUM, &conf)); // 1. Config
			ESP_LOGI(TAG, "i2c_driver_install %d", I2C_MASTER_NUM);
		    //ESP_ERROR_CHECK(i2c_driver_install(I2C_MASTER_NUM, conf.mode, I2C_MASTER_RX_BUF_DISABLE, I2C_MASTER_TX_BUF_DISABLE, 0)); // 2. driver install
			break;
		}
...
uint8_t u8g2_esp32_gpio_and_delay_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) {
	...
	// Set the GPIO reset pin to the value passed in through arg_int.
		case U8X8_MSG_GPIO_RESET:
			if (u8g2_esp32_hal.reset != U8G2_ESP32_HAL_UNDEFINED) {
				ESP_LOGI(TAG, "U8X8_MSG_GPIO_RESET %d %d", u8g2_esp32_hal.reset, arg_int);
